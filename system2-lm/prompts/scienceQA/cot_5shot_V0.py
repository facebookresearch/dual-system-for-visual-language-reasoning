...
Pretzel.
Potato chips, ocean water and pretzel are all salty. So the answer is (c).
...
North.
Poles that are the same repel. So the answer is (b).
...
16 u.
The particles in both samples have the same average speed, but each particle in sample A has more mass than each particle in sample B. Thus, sample A must have the higher temperature. So the answer is (a). 
...
Europe.
The continent in dark green is Europe. So the answer is (c).
...
Frog.
Most kinds of frogs' feet are adapted for sticking to smooth surfaces, including tiger-striped leaf frog. So the answer is (b).
