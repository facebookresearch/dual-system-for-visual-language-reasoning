...
def main(args):
    ...
    for example in raw_predictions:
        ...
        if processed_prediction is None or processed_answer is None:
            ...
            if processed_answer != 0:
                if (
                    abs(processed_prediction - processed_answer) / abs(processed_answer)
                    <= 0.05
                ):
                    relax_accuracy += 1
                    debug_flag = 1
    ...
    with open(result_file, "w") as fw:
        json.dump(
            {
                "exact_accuracy": exact_accuracy * 100.0,
                "relax_accuracy": relax_accuracy * 100.0,
                "accuracy_by_type": exact_accuracy_by_type,
            },
            fw,
            indent=4,
        )
